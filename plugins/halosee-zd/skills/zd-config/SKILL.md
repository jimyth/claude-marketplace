...
```bash
# 设置服务器地址
npx tsx "$ZD_SCRIPT" config --url http://localhost:8080

# 设置账号密码
npx tsx "$ZD_SCRIPT" config --account your_account
npx tsx "$ZD_SCRIPT" config --password your_password

# 设置工时统计周期（每月几号为周期起始日，默认24）
npx tsx "$ZD_SCRIPT" config --cycle 24

# 查看当前配置
npx tsx "$ZD_SCRIPT" config --show

# 测试连接
npx tsx "$ZD_SCRIPT" config --test
```
...
# 2. 设置账号密码
npx tsx "$ZD_SCRIPT" config --account jimyth
npx tsx "$ZD_SCRIPT" config --password your_password
